...
class DB{
    ...
    {
        $pass == '' ? self::init($host, $db, $user, $pass) : self::init($host, $db, $user, ''); 
    }
    ...
    public function updateObject($tabla, $set, $condicion, $params = ''){
        ...
        try{
            ...
            echo "Hubo un error al actualizar el registro en la tabla: ". $e->getMessage(). "<br>";
        }finally{
            return $res;
        }
    }
    ...
    public function selectAllObjects($nombreTabla, $innerJoin = ''){

        $this->prepareSQL('SELECT * FROM '.$nombreTabla. ' '. $innerJoin);
        ...
        try{
            ...
            while($objeto = DB::$sentencia->fetchAll(PDO::FETCH_CLASS, $nombreTabla)){
                $listaObjetos = $objeto;
            }
        }catch(PDOException $e){
            echo "Hubo un error al generar la consulta: ". $e->getMessage(). "<br>";
